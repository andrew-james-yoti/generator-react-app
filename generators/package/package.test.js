...
describe('app', () => {
    ...
    it('should add the application name', (done) => {
        helpers.run(path.join(__dirname, '../package'))
            ...
            .then(() => {
                assert.jsonFileContent(path.join(__dirname, 'tmp/test-app-name/package.json'), { name: 'test-app-name' });
                done();
            });
    });
    ...
    it('should add redux to the dependencies', (done) => {
        helpers.run(path.join(__dirname, '../package'))
            ...
            .then(() => {
                assert.jsonFileContent(path.join(__dirname, 'tmp/test-app-name/package.json'), {
                    dependencies: {
                        react: '16.9.0',
                        'react-dom': '16.9.0',
    ...
    it('should not add redux to the dependencies', (done) => {
        helpers.run(path.join(__dirname, '../package'))
            ...
            .then(() => {
                assert.noJsonFileContent(path.join(__dirname, 'tmp/test-app-name/package.json'), {
                    dependencies: {
                        'react-redux': '7.1.1',
                        redux: '4.0.4',
                        'redux-thunk': '2.3.0'
